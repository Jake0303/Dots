        ...
        else
        {
            ...
            else if (sp_triggerOnButtonClick.boolValue)
            {
                ...
                if (string.IsNullOrEmpty(sp_buttonName.stringValue)
                    || sp_buttonName.stringValue.Equals(DoozyUI.UIManager.DEFAULT_BUTTON_NAME)
                    && sp_dispatchAll.boolValue == false)
                {
                ...
                if (sp_dispatchAll.boolValue == false)
                {
                    ...
                    if(DoozyUI.UIManager.GetIndexForButtonName(sp_buttonName.stringValue) == -1) //we have a button name that is not in the database; we reset this value
                    {
                        sp_buttonName.stringValue = DoozyUI.UIManager.DEFAULT_BUTTON_NAME;
                    }

                    if (sp_buttonName.stringValue.Equals(DoozyUI.UIManager.DEFAULT_BUTTON_NAME))
                    {
                        DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightRed);
                    }
                    else
                    ...
                    buttonNameCurrentIndex = DoozyUI.UIManager.GetIndexForButtonName(sp_buttonName.stringValue);
